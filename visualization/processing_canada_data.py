...
with open(OUTPUT_PROP_FILE, 'r') as f:
    cluster_props = json.load(f)

file_class_one = 'class_one_new_data.docx'
file_class_two = 'class_two_new_data.docx'
file_class_three = 'class_three_new_data.docx'

csv_class1 = 'class_one_new_data.csv'
...
cluster_metric, clusters, cluster_word_list = zip(*sorted(zip(cluster_metric, clusters, 
    cluster_word_list)))
...
def get_para_object(count, phone, body, word_list, par):
    run = par.add_run("{}, Phone: {}".format(count, str(phone)))
    run.font.size = Pt(7)
    run.font.bold = True
...
for ind, cl in enumerate(clusters):
    ...
    try:
        ...
        if len(df_filt) > 100:
            ...
            continue
        class1_clusters.append([cl, round(cluster_metric[ind],2)])
        ...
        run = par.runs[0]
        run.font.size = Pt(11)
        par = doc.add_paragraph('{} ads, Cluster Metric = {}'.format(len(df_filt),
            round(cluster_metric[ind],2)))
        run = par.runs[0]
        run.font.size = Pt(7)
        ...
        for i, row in df_filt.iterrows():
            ...
            get_para_object(str(count), row['phone'], row['body'], cluster_word_list[ind].split(", ")[:20], par)
        doc.add_page_break()
